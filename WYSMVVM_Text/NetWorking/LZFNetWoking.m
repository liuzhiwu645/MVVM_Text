...
@implementation LZFNetWoking

+ (LZFNetWoking *)sharedLXNetworking
{
    static LZFNetWoking *handler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[LZFNetWoking alloc] init];
    });
    return handler;
...
//            return;
    ...
    AFHTTPSessionManager *manager = [LZFNetWoking getAFManager];
    ...
    [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formData) {
        ...
        for (UIImage *image in images) {
            NSString *fileName = [NSString
                                  stringWithFormat:@"%@%d.png",dateStr,i];
            NSData *imageData;
            ...
            [formData appendPartWithFileData:imageData name:parameter
                                    fileName:fileName mimeType:@"image/jpg/png/jpeg"];
        }
...
+(LXURLSessionTask *)baseRequestType:(NSUInteger)type
    ...
    //拼接URL
    NSString *urlStr1 = [base_Url stringByAppendingString:url];
    ...
    //检查地址中是否有中文
    NSString *urlStr=[NSURL URLWithString:urlStr1]?urlStr1:[self strUTF8Encoding:urlStr1];
...
//        [dict setValue:strData forKey:@"inputParameter"];
    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                DLog(@"未知网络");
                [LZFNetWoking sharedLXNetworking].networkStats=StatusUnknown;
                
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                DLog(@"没有网络");
                [LZFNetWoking sharedLXNetworking].networkStats=StatusNotReachable;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN: // 手机自带网络
                DLog(@"手机自带网络");
                [LZFNetWoking sharedLXNetworking].networkStats=StatusReachableViaWWAN;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi: // WIFI
                
                [LZFNetWoking sharedLXNetworking].networkStats=StatusReachableViaWiFi;
                DLog(@"WIFI--%d",[LZFNetWoking sharedLXNetworking].networkStats);
                break;
        }
    }];
    [mgr startMonitoring];
